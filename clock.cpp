...
void Clock::Draw() const {
	...
	DrawCircleV(position, 15, LIGHT_GREY);
}
...
void Clock::DrawFace() const {
	...
	DrawCircleV(position, size, RAYWHITE);
	//DrawCircleV(position, size - 30, LIGHT_GREY);
	DrawCircleV(position, size - 30, DARK_BLACK);
	//DrawCircleV(position, size - 40, RAYWHITE);
	DrawCircleV(position, size - 40, DARK_BLACK);
}
...
void Clock::DrawHourMarks() const {
	...
	for (int i = 0; i < 12; i++) {
		...
		DrawRectanglePro(rect, { rectWidth / 2, rectHeight }, i * 30, RAYWHITE);
	}
	//DrawCircleV(position, size - 50, RAYWHITE);
	DrawCircleV(position, size - 50, DARK_BLACK);
}
...
void Clock::DrawMinuteHand(int minute) const {
	...
	DrawHand(handWidth, handLength, angle, RAYWHITE, 0);
}

void Clock::DrawHourHand(int hour, int minute) const {
	float handWidth = 15;
	float handLength = size * 0.55;
...
void Clock::DrawSecondHand(int second) const {
	...
	DrawHand(handWidth, handLength, angle, BLUE, 55);
}
...
void Clock::DrawHand(float handWidth, float handLength, int angle, Color color, int offset) const {
	...
	Vector2 tipBase = { position.x + (handLength - offset) * sinf(DEG2RAD * angle), position.y - (handLength - offset) * cosf(DEG2RAD * angle) };
	Vector2 tipLeft = { tipBase.x - (handWidth / 2) * cosf(DEG2RAD * angle), tipBase.y - (handWidth / 2) * sinf(DEG2RAD * angle) };
	Vector2 tipRight = { tipBase.x + (handWidth / 2) * cosf(DEG2RAD * angle), tipBase.y + (handWidth / 2) * sinf(DEG2RAD * angle) };
	Vector2 tipPoint = { tipBase.x + tipLength * sinf(DEG2RAD * angle), tipBase.y - tipLength * cosf(DEG2RAD * angle) };
	...
	DrawRectanglePro(shadowRect, { handWidth / 2, handLength - offset }, angle, ColorAlpha(RAYWHITE, 0.3f));
}
...
void Clock::DrawHourLabels() const {
	...
	for (int i = 0; i < 4; i++) {
		...
		float theta_rad = theta_deg * DEG2RAD;
		...
		DrawText(text, x - textWidth / 2, y - textHeight / 2, fontSize, RAYWHITE);
	}
}
